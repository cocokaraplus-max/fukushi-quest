...
4. **★続けて `supabase_migration_20260824.sql` も同じ手順で Run（1500人規模むけの追加。2026-08-24）。**
   - 認識番号の連番発行（`issue_recog`）と、ガチャ回数の上限（1端末3回）が入る。
   - **これを流さないと、認識番号が「仮」のままになり、回数の上限も効かない。**
   - すでにデータがあっても消えない。2回以上 Run しても壊れない。
   - 詳しくは `README.md` の「2026-08-24 の変更」を読むこと。
